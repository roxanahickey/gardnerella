...
# This script follows part 1 and the step in which genome groups are specified. This can be done using hierarchical clustering and/or phylogenetic assessment. In the case of 35 Gardnerella vaginalis genomes, I performed both hierarchical clustering of ortholog presence/absence and phylogenetic concordance analysis of the core genes, settling on a grouping of 5 clades. This script follows that by extracting FASTA IDs and annotations for each clade. Finally, it produces summary figures of core and pan genome CDS counts.
...
genome.key$hc_group <- cutg[genome.key$id_short]

# bucky.clade.id <- read.table("data/reference/bucky_clade_ids.txt", header=T)
# bc <- unlist(bucky.clade.id$clade_id)
# names(bc) <- unlist(bucky.clade.id$id_short)

# genome.key <- merge(genome.key, bucky.clade.id)

## pick whether to group by hclust or phylogenetic clade (SELECT ONE)
# gp <- bc
gp <- cutg
...
#   group.tmp <- paste0("group", as.character(genome.key$clade_id[genome.key$id_short==i]))
  group.tmp <- paste0("group", as.character(genome.key$hc_group[genome.key$id_short==i]))
