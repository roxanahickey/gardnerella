...
dir.out <- "results/orthomcl70-gv35-bif22"
dir.fig <- "results/figures/orthomcl70-gv35-bif22"

## read in ID key containing short and full strain names
genome.key <- read.table("data/reference/key_gv35_bif22_id.txt", sep="\t", header=T)

## read in orthoMCL groups
ortho.clust.in <- readLines("data/reference/orthomcl70_gv35_bif22_groups.txt")
ortho.sing.in <- readLines("data/reference/orthomcl70_gv35_bif22_single.txt")
clust.mx.in <- read.table("data/reference/orthomcl70_gv35_bif22_groups_binary.txt", header=T)

## key for fasta headers
key.fasta.gv <- read.table("data/reference/key_gv35_fid_cds_fasta.txt", header=T, sep="\t")
key.fasta.bif <- read.table("data/reference/key_bif22_fid_cds_fasta.txt", header=T, sep="\t")
...
key.feat.bif <- read.table("data/reference/anno_bif22.features.tab", header=T, sep="\t", quote="")
...
key.ec.bif <- read.table("data/reference/anno_bif22.ec", header=T, sep="\t", quote="")
...
key.figfam.bif <- read.table("data/reference/anno_bif22.figfam", header=T, sep="\t", quote="")
...
key.go.bif <- read.table("data/reference/anno_bif22.go", header=T, sep="\t", quote="")
...
key.path.bif <- read.table("data/reference/anno_bif22.path", header=T, sep="\t", quote="")
