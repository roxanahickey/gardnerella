...
feat.key <- read.table("data/gv_patric_all35.features.tab", sep="\t", header=T, quote="")
feat.key$short_id <- paste("fid", feat.key$na_feature_id, "locus", feat.key$locus_tag, sep="|")
feat.master <- merge(feat.key, master.key[,c("short_id", "strain_id", "cds_id", "ortho_id")])
...
##################################################
## write singleton gene cluster feature/anno info per genome
# initialize files (will append results in loop below)
master.key.hdr <- data.frame(matrix("NA", nrow=0, ncol=ncol(master.key)+1))
colnames(master.key.hdr) <- c(colnames(master.key), "id_short")
write.table(master.key.hdr, file="output/orthomcl-match-anno/strain_uniq_info.txt",
            sep="\t", row.names=F, quote=F)

anno.master.hdr <- data.frame(matrix("NA", nrow=0, ncol=ncol(anno.master)+1))
colnames(anno.master.hdr) <- c(colnames(anno.master), "id_short")
write.table(anno.master.hdr, file="output/orthomcl-match-anno/strain_uniq_anno.txt",
            sep="\t", row.names=F, quote=F)

for(i in unique(ortho.sing.strain)){
  sing.tmp <- ortho.sing.id[grep(i, ortho.sing)]
  
  sing.info.tmp <- master.key[master.key$ortho_id %in% sing.tmp,]
  sing.info.tmp$id_short <- i
  
  write.table(sing.info.tmp, file="output/orthomcl-match-anno/strain_uniq_info.txt", 
              sep="\t", append=TRUE, row.names=F, col.names=F, quote=F)
  
  sing.anno.tmp <- anno.master[anno.master$ortho_id %in% sing.tmp,]
  sing.anno.tmp$id_short <- i

  write.table(sing.anno.tmp, file="output/orthomcl-match-anno/strain_uniq_anno.txt",  
              sep="\t", append=TRUE, row.names=F, col.names=F, quote=F)
}

##################################################
## clean up, save image
rm(pick, i, sing.tmp, sing.info.tmp, sing.anno.tmp)
